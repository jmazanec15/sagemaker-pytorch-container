""" Script to create SageMaker PyTorch Docker images
    ...
    Usage:
        python docker_image_creator.py optimized_binary_link gpu|cpu pytorch_version python_version
"""
import argparse
import glob
import os
...
def create_docker_image(optbin_link, processor, framework_version, python_version):
    """ Function builds a docker image with the PT optimized binary
    ...
    optbin_filename = 'pytorch-{}-cp27-cp27mu-manylinux1_x86_64.whl'.format(framework_version)
    with open('{}/../docker/{}/final/{}'.format(PATH_TO_SCRIPT, framework_version, optbin_filename), 'wb') as optbin_file:
    ...
    output_file = glob.glob('{}/../dist/sagemaker_pytorch_container-*.tar.gz'.format(PATH_TO_SCRIPT))[0] # use glob to use regex
    output_filename = output_file.split('/')[-1]
    shutil.copyfile(output_file, '{}/../docker/{}/final/{}'.format(PATH_TO_SCRIPT, framework_version, output_filename))
    subprocess.call([DOCKER, 'build', '-t', 'preprod-pytorch:{}-{}-{}'.format(framework_version, processor, py_v), \
                    '--build-arg', 'py_version={}'.format(py_v[-1]), '--build-arg', 'framework_installable={}'.format(optbin_filename), \
                    '-f', 'Dockerfile.{}'.format(processor), '.'], cwd='{}/../docker/{}/final'.format(PATH_TO_SCRIPT, framework_version))
...
if __name__ == '__main__':
    ...
    parser.add_argument('framework_version', help='PyTorch framework version (i.e. 0.4.0)')
